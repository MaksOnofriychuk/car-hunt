...
- Усе пристроєве — сесія, автор, вигляд, режим черги — живе в `localStorage`, а
  cookie з тим самим імʼям лише везе значення на сервер (`src/lib/device-store.ts`).
  Скрипт у `<head>` відновлює зниклу cookie зі сховища до першого малювання: саме
  зникла cookie вибивала з застосунку на телефоні.
- Налаштування діляться за місцем життя: вигляд (тема, розмір, шрифт, щільність)
  — `car_hunt_look` на пристрої; робота і сповіщення — таблиця
...
- Режим черги (список/таблиця) і набір колонок — у `car_hunt_view`:
  сервер читає значення при рендері, тому таблиця малюється одразу. Таблиця — тільки
...
- Telegram-сповіщення йде **обом**, і автору дії теж: машина шукається на двох.
  Перемикачі в `user_settings` — особисті, вони кажуть лише про свого отримувача.
